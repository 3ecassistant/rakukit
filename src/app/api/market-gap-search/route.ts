...
import { analyzeMarketGaps } from "@/lib/marketGapFinder";
import { normalizeItem } from "@/lib/rakutenItem";

export const runtime = "nodejs";

const ALLOWED_COUNTS = new Set([100, 300, 1000]);
...
export async function POST(req: NextRequest) {
  ...
  if (!genreId) {
    return NextResponse.json({ error: "ジャンルIDを入力してください" }, { status: 400 });
  }

  const requestedCount = ALLOWED_COUNTS.has(body.requestedCount as number) ? (body.requestedCount as 100 | 300 | 1000) : 300;
  ...
  try {
    ...
    const analysis = analyzeMarketGaps(items, attributeMaster);
